...
能否向任意方向做投影得到计算公式？比如，考虑下面的简单图形：

$$
P(v_1, v_2, \cdots, v_n) = \left\{ \sum_{i=1}^n x_i v_i \in \mathbb{R}^n \mid 0 \leq x_i \leq 1,\ i = 1, 2, \cdots, n. \right\}
$$

其中 $v_i$ ($1 \leq i \leq n$) 为 $\mathbb{R}^n$ 中的向量. 当 $n = 2$ 且 $v_1, v_2$ 线性无关时，$P(v_1, v_2)$ 为平面上的平行四边形. 如果这个平行四边形有一条边和坐标轴平行，则用投影法可以得出其面积公式. 一般的情形如何处理？
...
我们在前面几节所考虑的重积分有两个局限：一是积分区域要是可求体积（面积）的有界集合，二是被积函数要是有界的. 现在我们看看怎样放宽这两个限制条件. 
